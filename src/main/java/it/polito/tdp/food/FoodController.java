...
public class FoodController {
    ...
    void doCreaGrafo(ActionEvent event) {
    	...
    	try {
    		int por = Integer.parseInt(porzioni);
    		List<Food> cibi = this.model.getFoods(por);
    		
    		this.boxFood.getItems().clear(); //La svuoto nel caso non fosse 'pulita'
    		this.boxFood.getItems().addAll(cibi);
    		
    	} catch(NumberFormatException e) {
    		this.txtResult.appendText("Errore, devi inserire un numero");
    		return;
    	}
    ...
    void doSimula(ActionEvent event) {
    	...
    	try {
    		...
    		this.txtResult.appendText("Inserisci un valore valido di K");
    		return;
    	}
